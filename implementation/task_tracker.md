...
| M2-IT-010 | Métricas sin cookies — Cloudflare Web Analytics | M2 | **REVIEW (parcial)** | `ninguna` | `claude` | — | ✓ 16/16 evals, 80/80 tests, build limpio | [ ] | Manuel pidió 25 sep "añade las cookies también"; aclarado por pregunta directa: quería métricas, no cookies de verdad — Cloudflare Web Analytics (gratis, sin cookies, todo ya vive en Cloudflare) en vez de Google Analytics, para no tener que montar un banner aceptar/rechazar nuevo. `site.analytics.cloudflareToken` vacío por defecto: el beacon (`src/components/CloudflareAnalytics.tsx`) no renderiza nada hasta que Manuel dé de alta el sitio en el dashboard de Cloudflare y pegue el token — **eso es lo único que falta, y solo lo puede hacer él**. CSP de `next.config.mjs` ampliada con los dos hosts reales que exige el beacon (`static.cloudflareinsights.com` en `script-src`, `cloudflareinsights.com` en `connect-src`), verificados contra la documentación oficial, no adivinados — `EV-011` (hosts concretos, nunca comodín) sigue en verde. `/cookies` y `/en/cookies` explican la herramienta como lo que es: sin cookies, sin necesidad de consentimiento bajo el art. 22.2 LSSI-CE, con enlace a la política de Cloudflare |
