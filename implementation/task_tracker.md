...
| M2-IT-009 | Quitar precios de todo, Instagram real, tono humano + Granada, biografía real, QR más grande en impresos | M2 | **DONE** | `copywriting` (no cargada) | `sonnet` (Codex se lanzó en segundo plano; Manuel pidió seguir con Claude antes de que devolviera nada — nunca llegó a producir diff, nada que duplicar) | — | ✓ 16/16 evals, 77/77 tests, build limpio | ✓ | Manuel pidió 25 sep: retira la tarifa publicada en `M2-IT-006` (reversión de negocio, no bug — de la web, del `priceRange` en el JSON-LD de `HomePage.tsx`, y de los dos impresos), Instagram real (`viola.granada`), tono más cercano + Granada en vez de España en metadatos y copy, biografía nueva con conservatorio/orquestas/bodas/+20 años, y QR más grande en cartel (42→58mm) y tarjeta (18→24mm) — verificado con capturas Playwright antes de darlo por bueno, no solo por el número en el CSS. **Piano/guitarra en la biografía sin tocar `EV-013`**: se leyó el eval real antes de escribir nada — no es un bloqueo ciego de la palabra, persigue frases-promesa concretas; una mención de formación no encaja en ningún patrón. `pricing.test.tsx` retirado; `anclas-vivas.test.tsx` actualizado para el nuevo destino del CTA (`#contacto`, no `#tarifa`). **Review 25 sep (revisor+crítico)**: el crítico ejecutó los 6 regex de `EV-013` contra 8 frases-promesa adversarias y encontró que 4 escapaban (artículo entre conjunción y "viola", "two instruments" nunca podía enganchar por mezcla de idioma, lista con coma sin cubrir) — el eval pasaba por razón correcta con la biografía real, pero con una lista de frases enumerada, el mismo antipatrón un nivel más abajo. `EV-013` v3 arregla los 4 huecos, verificado con las 8 frases + la biografía real antes y después. Veredicto: avanzar |
| M2-IT-010 | Métricas sin cookies — Cloudflare Web Analytics | M2 | **REVIEW (parcial)** | `ninguna` | `sonnet` | — | ✓ 16/16 evals, 77/77 tests, build limpio | ✓ | Manuel pidió 25 sep "añade las cookies también"; aclarado por pregunta directa: quería métricas, no cookies de verdad — Cloudflare Web Analytics (gratis, sin cookies, todo ya vive en Cloudflare) en vez de Google Analytics. `site.analytics.cloudflareToken` vacío por defecto: el beacon no renderiza nada hasta que Manuel dé de alta el sitio y pegue el token — **eso es lo único que falta, y solo lo puede hacer él**. CSP ampliada con los dos hosts reales del beacon (fuente citada en `DEC-023`). `/cookies` y `/en/cookies` explican la herramienta bajo el art. 22.2 LSSI-CE. **Review 25 sep**: el revisor cazó una cifra falsa ("80/80 tests" cuando eran 75/75 — copiada sin re-ejecutar) y que `CloudflareAnalytics.tsx` no tenía ni un test; el crítico sostuvo el hallazgo y añadió que solo se había verificado el camino SIN token, no el camino CON token — el único que corre en producción. Arreglado: `tests/unit/cloudflare-analytics.test.tsx` nuevo, cubre los dos caminos (sin token no renderiza; con token, `src`/`defer`/`data-cf-beacon` exactos). **Aviso para cuando Manuel dé de alta el sitio**: elegir "manual setup" en el dashboard de Cloudflare, no "automatic" — automatic inyecta el beacon en el borde para toda zona proxiada (que un Worker siempre es) y duplicaría el conteo junto con este componente. Documentado en `site.ts` y en `DEC-023`. Veredicto: avanzar, parcial solo por el token pendiente de Manuel |
