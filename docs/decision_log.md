...
## DEC-022: Cloudflare Web Analytics en vez de Google Analytics

> Nota de numeración: esta rama (`feat/cloudflare-web-analytics`) y
> `feat/contenido-humano-sin-precios` salieron las dos de `main` el mismo día y las dos
> añaden un `DEC-022` — colisión esperada de dos PR en paralelo sobre un fichero que solo
> crece por el final. Se resuelve renumerando uno a `DEC-023` en el merge que llegue
> segundo; no es un error de ninguna de las dos entradas.
